...
create(artifact_id=None, meta_props=None, rev_id=None, **kwargs)
...
meta_props (dict) - meta props. To see the available list of metanames, use client.deployments.ScoringMetaNames.get() or client.deployments.DecisionOptimizationMetaNames.get()
...
delete(deployment_id=None, **kwargs)
Delete a deployment.
...
delete_job(job_id=None, hard_delete=False, **kwargs)
...
generate(deployment_id, prompt=None, params=None, guardrails=False, guardrails_hap_params=None, guardrails_pii_params=None, concurrency_limit=8, async_mode=False, validate_prompt_variables=True, guardrails_granite_guardian_params=None)
...
generate_text(deployment_id, prompt=None, params=None, raw_response=False, guardrails=False, guardrails_hap_params=None, guardrails_pii_params=None, concurrency_limit=8, validate_prompt_variables=True, guardrails_granite_guardian_params=None)
...
generate_text_stream(deployment_id, prompt=None, params=None, raw_response=False, guardrails=False, guardrails_hap_params=None, guardrails_pii_params=None, validate_prompt_variables=True, guardrails_granite_guardian_params=None)
...
warnings.filterwarnings("always", category=HAPDetectionWarning)
get_details(deployment_id=None, serving_name=None, limit=None, asynchronous=False, get_all=False, spec_state=None, attempt_activation=None, _silent=False, **kwargs)
...
attempt_activation (bool, optional) - whether to activate the deployment (wake it up with given deployment_id)

Returns:
metadata of the deployment(s)

Return type:
dict (if deployment_id is not None) or {"resources": [dict]} otherwise
...
Retrieve multiple deployments

Retrieval using Generator
...
get_job_details(job_id=None, include=None, limit=None, **kwargs)
...
is_serving_name_available(serving_name)
...
list(limit=None, artifact_type=None, include_software_spec_state=True)
...
include_software_spec_state (bool, optional) - include 'SPEC_STATE' and 'SPEC_REPLACEMENT' columns in deployments. This requires sending more requests and slows down execution, defaults to True
...
run_ai_service(deployment_id, ai_service_payload, path_suffix=None)
...
run_ai_service_stream(deployment_id, ai_service_payload)
Execute an AI service by providing a scoring payload.
...
update(deployment_id=None, changes=None, background_mode=False, **kwargs)
...
----- ----- ----- ----- -----
class ibm_watsonx_ai.deployments.RuntimeContext(api_client, request_payload_json=None, method=None, path=None)

Class included to keep the interface compatible with the Deployment's RuntimeContext used in AIServices implementation.

Parameters:
api_client (APIClient) - initialized APIClient object with a set project ID or space ID. If passed, credentials and project_id/space_id are not required.
...
RuntimeContext example initialized for testing purposes before deployment:
...
get_json()
Get payload JSON send in body of API request to the generate or generate_stream method in deployed AIService. For testing purposes the payload JSON need to be set in RuntimeContext initialization or later as request_payload_json property.
...
ibm_watsonx_ai.deployments Source Code [FOR REFERENCE ONLY, DO NOT ATTEMPT TO EDIT, INCLUDE CHANGES TO, ETC. WORK WITHIN ITS CONSTRAINTS.]
